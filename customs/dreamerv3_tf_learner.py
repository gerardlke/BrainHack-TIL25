...
class DreamerV3TfLearner(DreamerV3Learner, TfLearner):
    ...
    def compute_gradients(
        ...
        **kwargs,
        ...
        for component in ["world_model", "actor", "critic"]:
            print(
                'self.metrics.peek((DEFAULT_MODULE_ID, component.upper() + "_L_total"))',
                self.metrics.peek((DEFAULT_MODULE_ID, component.upper() + "_L_total")),
                (DEFAULT_MODULE_ID, component.upper() + "_L_total"),
                type(self.metrics.peek((DEFAULT_MODULE_ID, component.upper() + "_L_total"))), 
            )
            print(
                'self.filter_param_dict_for_optimizer(self._params, self.get_optimizer(optimizer_name=component))',
                type(self.filter_param_dict_for_optimizer(self._params, self.get_optimizer(optimizer_name=component)),)
            )
            grads.update(
                gradient_tape.gradient(self.metrics.peek((DEFAULT_MODULE_ID, component.upper() + "_L_total")), self.filter_param_dict_for_optimizer(self._params, self.get_optimizer(optimizer_name=component)),)
            )
        del gradient_tape
        return grads
    ...
    def compute_loss_for_module(
        ...
        # print('prediction_losses', prediction_losses)
        # print('rewards_B_T', batch[Columns.REWARDS])
        # print('continues_B_T', (1.0 - tf.cast(batch["is_terminated"], tf.float32)))
        ...
        # print('L_dyn_B_T', L_dyn_B_T)
        # print('L_rep_B_T', L_rep_B_T)
        ...
        # print('L_world_model_total_B_T', L_world_model_total_B_T)
        ...
        # print('L_world_model_total', L_world_model_total)
        ...
        self.metrics.log_dict(
            ...
            key=module_id,
            window=1,  # <- single items (should not be mean/ema-reduced over time).
            reduce=None,  # NOTE: injected code!!
        )

        # print('test peek???--------------------------')
        # test = self.metrics.peek((DEFAULT_MODULE_ID, "WORLD_MODEL_L_total"))
        # print('test DEFAULT_MODULE_ID WORLD_MODEL_L_total', test, type(test))
        # print('but what i used to store was', L_world_model_total, type(L_world_model_total))
