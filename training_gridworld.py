...
class raw_env(AECEnv[AgentID, ObsType, ActionType]):
    ...
    def reset(self, seed=None, options=None):
        ...
        print('self.agent_selector', self.agent_selector)
        # if self.swap_roles:
        self.agent_selection = self.agent_selector.next()
        # select the next player to be the scout
        self.scout: AgentID = self._scout_selector.next()
        print('self.agent_selection', self.agent_selection)
        print('self.scout', self.scout)
        print('self._scout_selector.is_first()??', self._scout_selector.is_first())
    ...
    def step(self, action: ActionType):
        ...
        # NOTE ben: ahhh so it compiles an action based on the agent_selection at the current step
        # call. then, if it is the last, it acts each one during the function _handle_actions.
